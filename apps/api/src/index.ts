...
async function getTagsTemplates() {
	...
	try {
		if (isDev) {
			...
			const tags = await got.get('https://raw.githubusercontent.com/iarshman/rexify/main/apps/api/tags.json').text();
			const response = await got
				.get('https://raw.githubusercontent.com/iarshman/rexify/main/apps/api/templates.json')
				.text();
			await fs.writeFile('/app/templates.json', response);
...
async function autoUpdater() {
	// Updates are handled manually via the Settings dashboard
	return;
}
